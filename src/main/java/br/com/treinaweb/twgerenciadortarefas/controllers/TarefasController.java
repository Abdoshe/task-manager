	...
	public ModelAndView inserir(@Valid Tarefa tarefa, BindingResult result, HttpServletRequest request) {
		...
		if(tarefa.getDataExpiracao() == null) {
			...
			if(tarefa.getDataExpiracao().before(new Date())) {
				result.rejectValue("dataExpiracao", "tarefa.dataExpiracaoInvalida", "A data de expiração não pode ser maior que a data atual.");
			}			
		}
		if(result.hasErrors()) {
			mv.setViewName("tarefas/inserir");
			...
			String email = request.getUserPrincipal().getName();
			Usuario usuarioLogado = servicoUsuario.encontrarPorEmail(email);
